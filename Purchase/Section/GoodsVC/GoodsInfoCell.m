...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _goods_noLabel.font = [UIFont customFontOfSize:12];
        ...
        _sellLabel.textColor = SHALLOWGRAY;
        ...
        _storeLabel.textColor = SHALLOWGRAY;
        ...
        _waitToBuyLabel.textColor = SHALLOWGRAY;
        ...
        _puchaseLabel.textColor = SHALLOWGRAY;
        ...
        [_goods_noLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(_priceLabel.mas_bottom).with.offset(2);
            make.left.equalTo(_goodsImageView.mas_right).with.offset(10);
            make.right.equalTo(self.contentView).with.offset(-10);
        }];
        [_sellLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(_goods_noLabel.mas_bottom).with.offset(2);
        ...
        [_waitToBuyLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(_sellLabel.mas_bottom).with.offset(2);
            make.left.equalTo(_priceLabel.mas_left).with.offset(0);
            make.right.equalTo(_puchaseLabel.mas_left).with.offset(-5);
        }];
        [_puchaseLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(_waitToBuyLabel.mas_top).with.offset(0);
            make.left.equalTo(_sellLabel.mas_right).with.offset(5);
            make.right.equalTo(_cartButton.mas_left).with.offset(0);
...
- (void)setCellContentWithGoodsDic:(NSDictionary *)infoDic
{
    ...
    NSString *str0 = NSInternationalString(@"最近购入价", @"最近购入价");
    NSString *str1 = NSInternationalString(@"商品条码", @"商品条码");
    ...
    if (goodsModel.recent_price > 0.000000) {
        priceStr = [NSString stringWithFormat:@"%@：%.2f",str0,(float)goodsModel.recent_price];
    }else{
        priceStr = NSInternationalString(@"最近购入价：--", @"最近购入价：--");
    }
    NSString *goods_noStr;
    if (goodsModel.goods_no.length > 0) {
        goods_noStr = [NSString stringWithFormat:@"%@：%@",str1,goodsModel.goods_no];
    }else{
        goods_noStr = NSInternationalString(@"商品条码：--", @"商品条码：--");
    }
    ...
    [_priceLabel setText:priceStr afterInheritingLabelAttributesAndConfiguringWithBlock:^NSMutableAttributedString *(NSMutableAttributedString *mutableAttributedString) {
        NSRange range = NSMakeRange(str0.length+1, priceStr.length-str0.length-1);
        //设定可点击文字的的大小
        UIFont *systemFont = [UIFont customFontOfSize:12];
        CTFontRef font = CTFontCreateWithName((__bridge CFStringRef)systemFont.fontName, systemFont.pointSize, NULL);
        if (font) {
    ...
    [_goods_noLabel setText:goods_noStr afterInheritingLabelAttributesAndConfiguringWithBlock:^NSMutableAttributedString *(NSMutableAttributedString *mutableAttributedString) {
        NSRange range = NSMakeRange(str1.length+1, goods_noStr.length-str1.length-1);
        //设定可点击文字的的大小
        UIFont *systemFont = [UIFont customFontOfSize:12];
        CTFontRef font = CTFontCreateWithName((__bridge CFStringRef)systemFont.fontName, systemFont.pointSize, NULL);
        if (font) {
            [mutableAttributedString addAttribute:(NSString *)kCTFontAttributeName value:(__bridge id)font range:range];
            [mutableAttributedString addAttribute:(NSString*)kCTForegroundColorAttributeName value:(id)[SHALLOWBLACK CGColor] range:range];
            CFRelease(font);
        }
        return mutableAttributedString;
    }];
