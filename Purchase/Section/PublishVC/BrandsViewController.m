...
@interface BrandsViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([SAFE_STRING([brandDic objectForKey:@"brand_name"]) isEqualToString:self.brandStr]) {
    ...
    if (_searchBar == nil) {
        ...
        for(UIView *view in  [[[_searchBar subviews] objectAtIndex:0] subviews]) {
            if([view isKindOfClass:[NSClassFromString(@"UINavigationButton") class]]) {
                ...
                [cancel setTitleColor:NAVBARCOLOR forState:UIControlStateNormal];
            }
        }
    }
    return _searchBar;
